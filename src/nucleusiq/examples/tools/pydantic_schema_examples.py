...
search_tool = BaseTool.from_function(
    search, description="Search for information", args_schema=SearchInput
)
...
async def demonstrate_tools():
    ...
    result4 = await search_tool.execute(
        query="Python async programming", max_results=3, sort_by="date"
    )
    ...
    logger.info("=" * 60)
    logger.info(
        "1. Simple functions: Use BaseTool.from_function() - schema auto-generated"
    )
    logger.info(
        "2. Advanced schemas: Use args_schema parameter with Pydantic BaseModel"
    )

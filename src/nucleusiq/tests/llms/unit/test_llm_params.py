...
Covers:
 ...
 - Range validation (temperature, max_output_tokens, etc.)
...
class TestLLMParamsConstruction:
    ...
    def test_valid_construction(self):
        ...
        p = LLMParams(temperature=0.7, max_output_tokens=256, seed=42)
    ...
    def test_max_output_tokens_must_be_positive(self):
        """max_output_tokens >= 1."""
        LLMParams(max_output_tokens=1)
        with pytest.raises(ValidationError):
            LLMParams(max_output_tokens=0)
...
class TestTypoDetection:
    ...
    def test_typo_max_token(self):
        """Rejects max_token (should be max_output_tokens)."""
...
class TestMerge:
    ...
    def test_new_fields_added(self):
        base = LLMParams(temperature=0.5)
        override = LLMParams(seed=42, max_output_tokens=512)
        result = base.merge(override)
        assert result.temperature == 0.5
...
class TestResolveIntegration:
    ...
    def test_per_execute_overrides_config(self):
        config_p = LLMParams(temperature=0.3, max_output_tokens=100)

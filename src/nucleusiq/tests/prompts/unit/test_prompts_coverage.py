...
class TestAutoChainOfThought:
    ...
    def test_no_llm_raises(self):
        ...
        with pytest.raises(PromptConfigError, match="llm"):
            p.format_prompt(task="Do X", questions=["Q1"])

    def test_empty_task_raises(self):
        p = AutoChainOfThoughtPrompt(llm=MockLLM())
        # BasePrompt.format_prompt rejects empty required strings before subclass hook.
        with pytest.raises(PromptTemplateError, match="task"):
            p.format_prompt(task="", questions=["Q1"])

    def test_empty_questions_raises(self):
        p = AutoChainOfThoughtPrompt(llm=MockLLM())
        with pytest.raises(PromptConfigError, match="questions"):
...
class TestPromptFactory:
    ...
    def test_register_duplicate_raises(self):
        with pytest.raises(PromptConfigError, match="already registered"):
...
class TestBasePromptSaveLoad:
    ...
    def test_save_unsupported_format(self, tmp_path):
        p = ZeroShotPrompt()
        with pytest.raises(PromptConfigError, match="Unsupported"):
    ...
    def test_load_missing_type(self, tmp_path):
        ...
        with pytest.raises(PromptConfigError, match="_type"):
    ...
    def test_load_unknown_type(self, tmp_path):
        ...
        with pytest.raises(PromptConfigError, match="Unsupported prompt type"):
            ZeroShotPrompt.load(path)

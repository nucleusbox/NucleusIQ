...
class TestAgentPromptPrecedence:
    ...
    async def test_prompt_takes_precedence_in_messages(self, agent_with_prompt):
        ...
        messages = MessageBuilder.build(
            ...
            role=agent_with_prompt.role,
            objective=agent_with_prompt.objective,
        )
        ...
        assert "Perform calculations" not in (
    ...
    async def test_role_objective_used_when_no_prompt(self, agent_without_prompt):
        ...
        messages = MessageBuilder.build(
            task,
            prompt=agent_without_prompt.prompt,
            role=agent_without_prompt.role,
            objective=agent_without_prompt.objective,
        )

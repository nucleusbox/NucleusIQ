...
import json
import pytest
from nucleusiq.agents.components.critic import Critic, CritiqueResult, Verdict, _truncate
...
class TestReasoningVerification:
    ...
    def test_no_tool_requirement(self):
        prompt = Critic._build_reasoning_verification(
            "Explain gravity", "Gravity is a force...", "Some reasoning",
        )
    ...
    def test_includes_trace_when_present(self):
        prompt = Critic._build_reasoning_verification(
            "Task", "Answer", "The agent reasoned step by step...",
        )
...
class TestVerifierAnswerField:
    ...
    def test_critique_result_with_verifier_answer(self):
        cr = CritiqueResult(
            verdict=Verdict.PASS, verifier_answer="10.95",
        )

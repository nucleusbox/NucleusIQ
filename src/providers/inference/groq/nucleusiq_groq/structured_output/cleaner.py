"""JSON schema cleanup for Groq strict structured outputs (same rules as OpenAI)."""
...
def clean_schema_for_groq(schema: dict[str, Any]) -> dict[str, Any]:
    """Clean a Pydantic JSON schema for Groq ``response_format`` / json_schema."""
    ...
    if isinstance(obj, dict):
        if "$ref" in obj:
            ...
            if ref_path.startswith("#/$defs/"):
                ...
                if def_name in defs:
                    return _inline_refs(
                        clean_schema_for_groq(defs[def_name]),
                        defs,
                    )

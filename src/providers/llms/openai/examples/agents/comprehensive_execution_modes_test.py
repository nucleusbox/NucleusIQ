...
async def test_direct_mode():
    ...
    logger.info(f"   Tools provided: {len(agent.tools)} (will be ignored in DIRECT mode)")
    ...
    test_cases = [
        ...
        {
            "name": "Explanation",
            "task": Task(id="direct_3", objective="Explain what Python is in one sentence."),
...
async def test_autonomous_mode():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "task": Task(id="autonomous_3", objective="If I have 100 apples and give away 30, then multiply the remainder by 2, what do I get?"),
...
async def main():
    ...
    logger.info("  3. AUTONOMOUS mode (Gear 3) - Full reasoning loop (falls back to standard)")
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"DIRECT Mode:     {'✅ PASS' if results.get('direct') else '❌ FAIL'}")
        logger.info(f"STANDARD Mode:   {'✅ PASS' if results.get('standard') else '❌ FAIL'}")
        logger.info(f"AUTONOMOUS Mode: {'✅ PASS' if results.get('autonomous') else '❌ FAIL'}")
        logger.info(f"Mode Comparison: {'✅ PASS' if results.get('comparison') else '❌ FAIL'}")

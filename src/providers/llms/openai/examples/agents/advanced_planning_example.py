...
def create_advanced_math_tools():
    ...
    subtract_tool = BaseTool.from_function(subtract, description="Subtract second number from first.")
    divide_tool = BaseTool.from_function(divide, description="Divide first number by second.")
    power_tool = BaseTool.from_function(power, description="Raise base to the power of exponent.")
    sqrt_tool = BaseTool.from_function(sqrt, description="Calculate square root of a number.")
...
async def test_llm_planning():
    ...
    logger.info("=" * 80)
    logger.info("This test demonstrates how the agent generates complex plans using LLM")
    logger.info("Mode: AUTONOMOUS - Full reasoning loop with planning and self-correction")
    logger.info("-" * 80)
...
async def test_complex_workflow_planning():
    ...
    logger.info("   Mode: AUTONOMOUS (automatic planning + execution)")
    logger.info("   Note: Agent will automatically plan and execute this complex workflow")
...
async def test_plan_with_context_building():
    ...
    logger.info("=" * 80)
    logger.info("This test demonstrates how plan steps build context from previous steps")
    ...
    logger.info("   Note: Agent will automatically plan and handle context across steps")
...
async def test_plan_serialization():
    ...
    plan = Plan(
        steps=[
            PlanStep(step=1, action="add", args={"a": 5, "b": 3}, details="Add 5 and 3"),
            PlanStep(step=2, action="multiply", args={"a": 8, "b": 2}, details="Multiply result by 2"),
            PlanStep(step=3, action="subtract", args={"a": 16, "b": 4}, details="Subtract 4"),
        ],
        task=task
    )

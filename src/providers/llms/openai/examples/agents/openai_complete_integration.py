...
def create_calculator_tools():
    ...
    area_tool = BaseTool.from_function(
        calculate_area, description="Calculate rectangle area"
    )
...
async def example_standard_mode_tools():
    ...
    task1 = Task(id="calc1", objective="What is 15 + 27? Use the add tool.")
...
async def example_standard_mode_openai_tools():
    ...
    task = Task(id="research", objective="Calculate what 2% of 14 million would be.")
...
async def example_structured_output_native():
    ...
    if isinstance(result, dict) and "output" in result:
        calc = result["output"]
        logger.info("📊 Structured Calculation Result:")
